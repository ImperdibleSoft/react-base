...
import { SHOW_NOTIFICATION, CLEAR_CACHE } from '../../common/constants/sw-message-types';
import { Message } from '../../definitions/message';
import { getWorkerConfig } from '../../common/utils/idb';

export const sendMessageToWebApp = async (message: Message) => {
...
const handler = async (event: any) => {
  ...
  switch (data.type) {
    ...
    case CLEAR_CACHE:
      ...
      return Promise.all(savedCaches.map(cache => caches.delete(cache)));
